...
city_anno_path = 'C:/Users/16415/WorkSpace/DataSet/CityPerson/annotations/'
city_img_path = 'C:/Users/16415/WorkSpace/DataSet/CityPerson/'
# train_file = 'custom_train.json'
train_file = 'custom_train_debug.json'
# val_file = 'custom_val.json'
val_file = 'custom_val_debug.json'
...
train_dataset = CocoDetection(img_path=city_img_path + 'train/', anno_path=city_anno_path, processor=processor,
                              train=True)
val_dataset = CocoDetection(img_path=city_img_path + 'train/', anno_path=city_anno_path, processor=processor,
                            train=False)
...
class CityDataLoader(DataLoader):
    def __init__(self, dataset, batch_size=4, shuffle=True, collate_fn=None, ):
        super().__init__(dataset, batch_size, shuffle, collate_fn=collate_fn)
        print("init CityDataLoader here.")


# set batch_size = 4 in original DETR
train_city_dataloader = CityDataLoader(train_dataset, collate_fn=collate_fn, batch_size=2, shuffle=True)
val_city_dataloader = CityDataLoader(val_dataset, collate_fn=collate_fn, batch_size=2)
batch = next(iter(train_city_dataloader))
...
class DeformableDetr(pl.LightningModule):
    ...
    def train_dataloader(self):
        return train_city_dataloader

    def val_dataloader(self):
        return val_city_dataloader
...
for idx, batch in enumerate(tqdm(val_city_dataloader)):
    # get the inputs
